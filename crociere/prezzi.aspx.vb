...
Partial Class crociere_prezzi
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If codiceperiodo = "PO20130923GOAGOA" Then
                ClientScript.RegisterStartupScript(Me.GetType(), "ClientScript", "altezza('" & altezza & "','frameprezzi'); iniaz('" & primocolore & "','" & secondocolore & "');" & rigona & divone & imapiu & testone & prezzone & prezzone2 & prezzone3 & prezzocolonna1 & prezzocolonna2 & prezzocolonna3 & manina1 & manina2 & manina3 & appoggio1 & appoggio2 & appoggio3 & "vedisotto('Repeatercategorie_divdescri_0','Repeatercategorie_riga_0', '241', '#F1F1F1', 'Repeatercategorie_ImageDetail_0', 'Repeatercategorie_descri_0', 'Repeatercategorie_Labelprezzo_0','1', '175');", True)
            Else
    ...
    Protected Sub Repeatercategorie_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeatercategorie.ItemCreated
        ...
        End If
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            Dim RepeaterprezziCat As Repeater = CType(e.Item.FindControl("RepeaterprezziCat"), Repeater)
            AddHandler RepeaterprezziCat.ItemDataBound, AddressOf RepeaterprezziCat_ItemDataBound
        End If
    End Sub

    Protected Sub RepeaterprezziCat_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs)
    ...
    Protected Sub Repeatercategorie_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeatercategorie.ItemDataBound
        ...
        Dim rigarq1 As System.Web.UI.HtmlControls.HtmlGenericControl = CType(e.Item.FindControl("rigarq1"), System.Web.UI.HtmlControls.HtmlGenericControl)
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Select Case codcat
                Case "I"
                    ...
                    tipocab = 2
                    filtraprezziCat(RepeaterprezziCat, "(codiceperiodo = '" & codiceperiodo.Text & "' OR codiceperiodo='" & primaperiodo & "' OR codiceperiodo = '" & dopoperiodo & "') AND (tipocabina >=1 and tipocabina <= 2)", co)
                    filtraprezzi(Repeaterprezzi, "codiceperiodo = '" & codiceperiodo.Text & "' AND (tipocabina >=1 and tipocabina <= 2)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezziprima, "codiceperiodo = '" & primaperiodo & "' AND (tipocabina >=1 and tipocabina <= 2)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezzidopo, "codiceperiodo = '" & dopoperiodo & "' AND (tipocabina >=1 and tipocabina <= 2)", RepeaterprezziCat, tipocab)
                Case "E"
                    epr = prezziperiodo(5)
                    epr2 = prezziperiodo(1)
                    epr3 = prezziperiodo(9)
                    tipocab = 4
                    filtraprezziCat(RepeaterprezziCat, "(codiceperiodo = '" & codiceperiodo.Text & "' OR codiceperiodo='" & primaperiodo & "' OR codiceperiodo = '" & dopoperiodo & "') AND (tipocabina >=3 and tipocabina <= 4)", co)
                    filtraprezzi(Repeaterprezzi, "codiceperiodo = '" & codiceperiodo.Text & "' AND (tipocabina >=3 and tipocabina <= 4)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezziprima, "codiceperiodo = '" & primaperiodo & "' AND (tipocabina >=3 and tipocabina <= 4)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezzidopo, "codiceperiodo = '" & dopoperiodo & "' AND (tipocabina >=3 and tipocabina <= 4)", RepeaterprezziCat, tipocab)
                Case "B"
                    epr = prezziperiodo(6)
                    epr2 = prezziperiodo(2)
                    epr3 = prezziperiodo(10)
                    tipocab = 6
                    filtraprezziCat(RepeaterprezziCat, "(codiceperiodo = '" & codiceperiodo.Text & "' OR codiceperiodo='" & primaperiodo & "' OR codiceperiodo = '" & dopoperiodo & "') AND (tipocabina >=5 and tipocabina <= 6)", co)
                    filtraprezzi(Repeaterprezzi, "codiceperiodo = '" & codiceperiodo.Text & "' AND (tipocabina >=5 and tipocabina <= 6)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezziprima, "codiceperiodo = '" & primaperiodo & "' AND (tipocabina >=5 and tipocabina <= 6)", RepeaterprezziCat, tipocab)
                    filtraprezzi(Repeaterprezzidopo, "codiceperiodo = '" & dopoperiodo & "' AND (tipocabina >=5 and tipocabina <= 6)", RepeaterprezziCat, tipocab)
                Case Else
                    If codcat = "S" Or codcat = "G" Or codcat = "M" Or codcat = "P" Or codcat = "W" Then
                        ...
                        tipocab = 8
                        filtraprezziCat(RepeaterprezziCat, "(codiceperiodo = '" & codiceperiodo.Text & "' OR codiceperiodo='" & primaperiodo & "' OR codiceperiodo = '" & dopoperiodo & "') AND (tipocabina >=7 and tipocabina <= 8)", co)
                        filtraprezzi(Repeaterprezzi, "codiceperiodo = '" & codiceperiodo.Text & "' AND (tipocabina >=7 and tipocabina <= 8)", RepeaterprezziCat, tipocab)
                        filtraprezzi(Repeaterprezziprima, "codiceperiodo = '" & primaperiodo & "' AND (tipocabina >=7 and tipocabina <= 8)", RepeaterprezziCat, tipocab)
                        filtraprezzi(Repeaterprezzidopo, "codiceperiodo = '" & dopoperiodo & "' AND (tipocabina >=7 and tipocabina <= 8)", RepeaterprezziCat, tipocab)
                    End If
            End Select
            ...
            If tutte = True Then
                rigarq1.Visible = True
                rigarq.Visible = True
                LabelRQ.Text = "per la data del " & Format(CDate(daltot), "dd/MM/yy") & " la " & descri.Text & "<br/>non è attualmente disponibile. Puoi inoltrare una<br/>richiesta nel caso si liverasse sarà nostra cura contattarti"
                sirq = True
            ...
            If tutte2 = True And Labelprezzo2.Text <> "" Then
                ...
                rigarq.Visible = True
                LabelRQ.Text = "per la data del " & Format(CDate(dalprima), "dd/MM/yy") & " la " & descri.Text & "<br/>non è attualmente disponibile. Puoi inoltrare una<br/>richiesta nel caso si liverasse sarà nostra cura contattarti"
                sirq = True
            ...
            If tutte3 = True And Labelprezzo3.Text <> "" Then
                ...
                rigarq.Visible = True
                LabelRQ.Text = "per la data del " & Format(CDate(daldopo), "dd/MM/yy") & " la " & descri.Text & "<br/>non è attualmente disponibile. Puoi inoltrare una<br/>richiesta nel caso si liverasse sarà nostra cura contattarti"
                sirq = True
    ...
    Private Sub filtraprezziCat(ByVal rpt As Repeater, ByVal filtro As String, ByVal compagnia As Integer)
        Dim dt As DataTable = dscat.Tables("Prezzi")
        Dim dv As New DataView(dt)
        dv.RowFilter = filtro
        If compagnia = 0 Then
            dv.Sort = "categoria, prezzo"
        ElseIf compagnia = 1 Then
            dv.Sort = "prezzo, categoria"
        End If
        Dim dt1 As DataTable
        dt1 = dv.ToTable
        Dim ds As New DsCosta
        Dim dtf As DataTable = ds.Tables("prezzi")
        For Each dr1 As DataRow In dt1.Rows
            Dim esiste As Boolean = False
            For Each dr2 As DataRow In dtf.Rows
                If dr1("categoria") = dr2("categoria") Then
                    esiste = True
                End If
            Next
            If esiste = False Then
                Dim drf As DataRow
                drf = dtf.NewRow
                drf("categoria") = dr1("categoria")
                drf("descrizione") = dr1("descrizione")
                dtf.Rows.Add(drf)
            End If
        Next
        rpt.DataSource = dtf
    ...
    Private Sub filtraprezzi(ByVal rpt As Repeater, ByVal filtro As String, ByVal rpt2 As Repeater, ByVal tipocabina As Integer)
        Dim dt As DataTable = dscat.Tables("Prezzi")
        Dim dv As New DataView(dt)
        dv.RowFilter = filtro
        Dim dt1 As DataTable
        ...
        For Each oitem In rpt2.Items
            Dim LabelCategoria As Label = CType(oitem.FindControl("LabelCategoria"), Label)
            Dim ce As Boolean = False
            Dim categoria As String = ""
            Dim disponibile As Integer = 0
            Dim prezzo As Integer = 0
            Dim codiceperiodo As String = ""
            For Each dr1 As DataRow In dt1.Rows
                If LabelCategoria.Text = dr1("categoria") Then                    
                    categoria = dr1("categoria")
                    disponibile = dr1("disponibile")
                    prezzo = dr1("prezzo")                    
                    codiceperiodo = dr1("codiceperiodo")
                    tipocabina = dr1("tipocabina")
                    ce = True
                End If
            Next
            If ce = True Then
                Dim drf As DataRow
                drf = dtf.NewRow
                drf("categoria") = categoria
                drf("disponibile") = disponibile
                drf("prezzo") = prezzo
                drf("codiceperiodo") = codiceperiodo
                drf("tipocabina") = tipocabina
                dtf.Rows.Add(drf)
            Else
                Dim drf As DataRow
                drf = dtf.NewRow
                drf("categoria") = LabelCategoria.Text
                drf("disponibile") = 1
                drf("prezzo") = 0
                drf("codiceperiodo") = ""
                drf("tipocabina") = tipocabina
