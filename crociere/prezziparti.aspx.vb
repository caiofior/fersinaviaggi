...
Partial Class crociere_prezzi
    ...
    Protected Sub Repeaterprezzi_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs)
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Else
                Dim stringacostap As String = "cabineparti.aspx"
    ...
    Protected Sub Repeatercategorie_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeatercategorie.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            If co = 0 Then 'Msc
                Select Case codcat
                    Case "I"
                        epr = esponiprezzo(codiceperiodo.Text, Repeaterprezzi, " AND tipocabina >= 1 and tipocabina <= 2 ORDER BY categoria")
                        epr2 = esponiprezzo(primaperiodo, Repeaterprezziprima, " AND tipocabina >= 1 and tipocabina <= 2 ORDER BY categoria")
                        epr3 = esponiprezzo(dopoperiodo, Repeaterprezzidopo, " AND tipocabina >= 1 and tipocabina <= 2 ORDER BY categoria")
                        tipocab = 2
                    Case "E"
                        epr = esponiprezzo(codiceperiodo.Text, Repeaterprezzi, " AND tipocabina >= 3 and tipocabina <= 4 ORDER BY categoria")
                        epr2 = esponiprezzo(primaperiodo, Repeaterprezziprima, " AND tipocabina >= 3 and tipocabina <= 4 ORDER BY categoria")
                        epr3 = esponiprezzo(dopoperiodo, Repeaterprezzidopo, " AND tipocabina >= 3 and tipocabina <= 4 ORDER BY categoria")
                        tipocab = 4
                    Case "B"
                        epr = esponiprezzo(codiceperiodo.Text, Repeaterprezzi, " AND tipocabina >= 5 and tipocabina <= 6 ORDER BY categoria")
                        epr2 = esponiprezzo(primaperiodo, Repeaterprezziprima, " AND tipocabina >= 5 and tipocabina <= 6 ORDER BY categoria")
                        epr3 = esponiprezzo(dopoperiodo, Repeaterprezzidopo, " AND tipocabina >= 5 and tipocabina <= 6 ORDER BY categoria")
                        tipocab = 6
                    Case "S"
                        epr = esponiprezzo(codiceperiodo.Text, Repeaterprezzi, " AND tipocabina >= 7 and tipocabina <= 8 ORDER BY categoria")
                        epr2 = esponiprezzo(primaperiodo, Repeaterprezziprima, " AND tipocabina >= 7 and tipocabina <= 8 ORDER BY categoria")
                        epr3 = esponiprezzo(dopoperiodo, Repeaterprezzidopo, " AND tipocabina >= 7 and tipocabina <= 8 ORDER BY categoria")
                        tipocab = 8
                End Select
            ElseIf co = 1 Then 'Costa
                If codcat = "S" Or codcat = "G" Or codcat = "M" Or codcat = "P" Or codcat = "W" Then
                    ...
                    tipocab = 8
                Else
                    If codcat = "I" Then tipocab = 2
                    If codcat = "E" Then tipocab = 4
                    If codcat = "B" Then tipocab = 6
    ...
    Function ricavacat(ByVal codiceperiodo As String, ByVal tipocabina As Integer) As String
        ricavacat = ""
        Dim sqlconn As String
        sqlconn = "SELECT * from prezzi WHERE codiceperiodo = '" & codiceperiodo & "' AND tipocabina = " & tipocabina & " ORDER BY categoria"
        Dim cmd As New MySqlCommand(sqlconn, cn2)
        If cn2.State = ConnectionState.Closed Then cn2.Open()
        Dim dr As MySqlDataReader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
        If dr.HasRows Then
            dr.Read()
            ricavacat = dr("categoria")
        End If
        dr.Close()
        cn2.Close()
    End Function
    ...
    Private Sub caricacat(ByVal codiceperiodo As String, ByVal ds As DsCosta, ByVal cat As String, ByVal descrizione As String, ByVal compagnia As Integer, ByVal tipomin As Integer, ByVal tipomax As Integer)
        Dim sqlconn As String = ""
        If compagnia = 0 Then 'msc
            'sqlconn = "SELECT codiceperiodo, '" & cat & "' As categoria, disponibile, listino, prezzo, '" & descrizione & "' As descrizione from prezzi WHERE disponibile = 0 AND codiceperiodo = '" & codiceperiodo & "' and tipocabina >= " & tipomin & " and tipocabina <= " & tipomax & " order by prezzo LIMIT 1"
            sqlconn = "SELECT codiceperiodo, '" & cat & "' As categoria, disponibile, listino, prezzo, '" & descrizione & "' As descrizione from prezzi WHERE disponibile = 2 AND codiceperiodo = '" & codiceperiodo & "' and tipocabina >= " & tipomin & " and tipocabina <= " & tipomax & " order by prezzo LIMIT 1"
        ElseIf compagnia = 1 Then 'costa
    ...
    Private Sub caricaprezzi(ByVal codiceperiodo As String, ByVal rpt As Repeater, ByVal filtro As String)
        Dim sqlconn As String
        sqlconn = "SELECT * from prezzi WHERE disponibile = 2 AND codiceperiodo = '" & codiceperiodo & "' " & filtro
        Dim cmd As New MySqlCommand(sqlconn, cn)
        If cn.State = ConnectionState.Closed Then cn.Open()
        Dim dr As MySqlDataReader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
        If dr.HasRows Then
    ...
    Function esponiprezzo(ByVal codiceperiodo As String, ByVal rpt As Repeater, ByVal filtro As String) As Integer
        esponiprezzo = 0
        Dim sqlconn As String
        sqlconn = "SELECT min(prezzo) as pr from prezzi WHERE disponibile = 2 AND codiceperiodo = '" & codiceperiodo & "' " & filtro
        Dim cmd As New MySqlCommand(sqlconn, cn)
        If cn.State = ConnectionState.Closed Then cn.Open()
        Dim dr As MySqlDataReader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
        If dr.HasRows Then
